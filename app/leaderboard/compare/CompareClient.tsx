  ...
  const comparison = useMemo(() => {
    ...
    const rows = Array.from(allIds)
      .map((matchId) => {
        ...
        const myRevealed =
          myPred?.homeScore !== null && myPred?.awayScore !== null;
        const otherRevealed =
          otherPred?.homeScore !== null && otherPred?.awayScore !== null;
        const same =
          myPred &&
          otherPred &&
          myRevealed &&
      ...
      {comparison && other && (
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            <Card
              className={cn(
                comparison.myPoints >= comparison.otherPoints &&
            ...
            >
              <CardContent className="px-3 py-3 text-center">
                {me?.image ? (
                  ...
                  <div className="w-7 h-7 rounded-full bg-muted flex items-center justify-center text-xs font-bold mx-auto mb-1">
                    {me?.name[0]}
                  </div>
                )}
            ...
            <Card
              className={cn(
                comparison.otherPoints > comparison.myPoints &&
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-1.5">
              {(["all", "same", "different"] as Filter[]).map((f) => (
                ...
                >
                  {f === "all"
                    ? `Todos (${comparison.total})`
                    : f === "same"
                      ? `Iguales (${comparison.same})`
                      : `Distintas (${comparison.different})`}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            {comparison.rows.map(
              ({ matchId, myPred, otherPred, match, same }) => {
                ...
                return (
                  ...
                  >
                    ...
                    {matchInfo && (
                      <div className="flex items-center justify-center gap-1.5 px-3 pt-1.5 pb-0.5 text-[10px] text-muted-foreground">
                        <span className="font-medium">
                          {translateCountry(matchInfo.home)}
                        </span>
                        <span>vs</span>
                        <span className="font-medium">
                          {translateCountry(matchInfo.away)}
                        </span>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 px-3 pb-2">
                      ...
                      >
                        {myPred &&
                        myPred.homeScore !== null &&
                        myPred.awayScore !== null ? (
                          <>
                            {myPred.points === 5 ? (
                      ...
                      <div className="flex items-center gap-1.5">
                        {otherPred &&
                        otherPred.homeScore !== null &&
                        otherPred.awayScore !== null ? (
                          <>
                            <span
                              className={cn(
                                "text-base font-black font-mono",
                    ...
                    {/* Points row */}
                    {(myPred?.points !== undefined ||
                      otherPred?.points !== undefined) && (
